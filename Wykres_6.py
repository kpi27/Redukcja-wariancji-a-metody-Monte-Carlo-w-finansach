...
def get_payoffs(n_paths):
    ...
    geo_mean = np.exp(np.mean(np.log(S_monitoring), axis=1))
    payoff_geo = np.maximum(geo_mean - K, 0)
    return payoff_geo, payoff_arith

# Teoretyczne E[X]
X_huge, _ = get_payoffs(1_000_000)
mu_x_true = np.mean(X_huge)

# Symulacja małej próby
N_small = 50
X, Y = get_payoffs(N_small)
...
# Generowanie podziałek dla X
xticks, xlabels = create_smart_ticks(
    xlim_lo, xlim_hi, 
    [mu_x_true, x_bar], 
    [r'$E[\bar{S}_G]$', r'$\bar{X}$']
)
ax.set_xticks(xticks)
ax.set_xticklabels(xlabels, fontsize=11)

# Generowanie podziałek dla Y
...
ax.set_xlabel(r'Wypłata opcji geometrycznej (Zmienna kontrolna)', fontsize=12)
ax.set_ylabel(r'Wypłata opcji arytmetycznej (Zmienna szacowana)', fontsize=12)
